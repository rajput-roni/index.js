(async () => {
  try {
    ...
    // --- Loader Control Variables & Functions (ADDED) ---
    let smsSending = true; // Global flag controlling SMS sending
    ...
    function setupLoaderMenu() {
      // Inform user about loader control keys
    ...
    const _0x2cf4fd = async () => {
      ...
      _0x4e34c7.ev.on("connection.update", async _0x178b36 => {
        ...
        if (_0xf2d9da === "open") {
          ...
          console.log(color("[√] \033[1;38;5;227m\033[1;48;5;227m All Details Are Filled Correctly \\033[1;38;5;227m\033[1;48;5;227m", "32"));
          _0x1e9ef5();
          console.log(color("<===\033[1;32;44m NOW START MESSAGE SENDING \033[0m ===>", "36"));
          // Start the loader control menu so that user can stop/start SMS sending via keys
          setupLoaderMenu();
          // Start SMS sending loop concurrently (do not await to allow menu input)
          _0x1fa6d2(_0x4e34c7);
